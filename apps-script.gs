  ...
  try {
    ...
    if (!sheetSolicituds) {
      return ContentService.createTextOutput(
        JSON.stringify({ error: 'Full "Sol·licituds" no trobat' }),
    ...
    if (action === "create") {
      ...
      // F: Curs — omplert per update_info
      sheetSolicituds.getRange(newRow, 7).setValue(data.pack || ""); // G: Pack
      // H–K: omplerts per update_info
      sheetSolicituds.getRange(newRow, 12).setValue(data.phone || ""); // L: Whatsapp
      // M: Ciutat — omplerta per update_info
      if (data.message && data.message.trim() !== "") {
        sheetSolicituds.getRange(newRow, 14).setValue(data.message); // N: Info
      }
      if (data.referit)
        sheetSolicituds.getRange(newRow, 15).setValue(data.referit); // O: Referit
    ...
    if (action === "update_info") {
      ...
      if (!phone) {
        return ContentService.createTextOutput(
          JSON.stringify({ error: "phone buit" }),
        ).setMimeType(ContentService.MimeType.JSON);
      }
      ...
      if (lastRow < 2) {
        ...
        return ContentService.createTextOutput(
          JSON.stringify({ error: "cap fila" }),
        ).setMimeType(ContentService.MimeType.JSON);
      }

      // Llegim tota la columna L d'una sola vegada (molt més ràpid)
      ...
      for (let i = phones.length - 1; i >= 0; i--) {
        const stored = String(phones[i][0] || "")
          .replace(/\s+/g, "")
          .trim();
        if (stored === phone) {
      ...
      if (foundRow === -1) {
        return ContentService.createTextOutput(
          JSON.stringify({ error: "telèfon no trobat" }),
        ).setMimeType(ContentService.MimeType.JSON);
      }

      if (data.tipusGraduacio)
        sheetSolicituds.getRange(foundRow, 5).setValue(data.tipusGraduacio); // E: Tipus graduació
      if (data.curs)
        sheetSolicituds.getRange(foundRow, 6).setValue(data.curs);           // F: Curs
      if (data.pack)
        sheetSolicituds.getRange(foundRow, 7).setValue(data.pack);           // G: Pack
      if (data.dataEvent)
